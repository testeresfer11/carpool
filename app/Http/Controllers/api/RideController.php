...
use App\Models\{Rides, Bookings, fare, User, Report, UserReport, Cars, Payments, SearchHistory, RideAlert, RefundPayment, Reviews, RewardSetting};
...
*/
    ...
    private function checkRideAlertsWithinRadius($ride)
    {
        ...
        foreach ($ride_alerts as $alert) {
            ...
            if ($departure_distance <= 50 || $arrival_distance <= 50) {
...
private function processStripeRefund($payment, $refundAmount)
{
            ...
            foreach ($rides as $ride) {
                ...
                foreach ($bookings as $booking) {
                   ...
                   // $commissionAmount = ($totalPaymentAmount * $commissionPercentage) / 100;
                    ...
                    $this->assignRewardIfEligible($user);
                    $this->assignRewardIfEligible($driver);
                    // Reward Points Functionality Ends Here
    ...
    private function assignRewardIfEligible($user)
    {
        $userType = $user->user_type; // 1=user, 2=driver
        $completedRides = $userType == 1
            ? \App\Models\Bookings::where('passenger_id', $user->user_id)->where('status', 'completed')->count()
            : \App\Models\Rides::where('driver_id', $user->user_id)->where('status', 2)->count();

        $rewardSettings = \App\Models\RewardSetting::where('user_type', $userType)->get();

        foreach ($rewardSettings as $setting) {
            $alreadyRewarded = \App\Models\RewardPoint::where('user_id', $user->user_id)
                ->where('reward_id', $setting->id)
                ->exists();
            ...
            $requiredRides = $userType == 1 ? $setting->user_completed_rides : $setting->driver_completed_rides;
            $rewardPoints  = $userType == 1 ? $setting->user_reward_points : $setting->driver_reward_points;

            if ($completedRides >= $requiredRides) {
                \App\Models\RewardPoint::create([
                    'user_id'         => $user->user_id,
                    'reward_id'       => $setting->id,
                    'user_type'       => $userType,
                    'received_points' => $rewardPoints,
                ...
                \Log::info("User ID {$user->user_id} (type {$userType}) awarded {$rewardPoints} points.");
            }
        }
    }
